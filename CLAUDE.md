...
**Group room names come from `KakaoRoomNameResolver`, nowhere else.** KakaoTalk 26.7.0 (2026-08) removed the room name from the notification body — `subText` is null and `conversationTitle` absent — so the resolver walks subText → conversationTitle → the conversation-shortcut label → a persisted `tag → name` map. Both the capture path and `ActiveNotificationFinder` (reply injection) must use it; if they ever disagree about a room's name, outbound injection fails silently. The shortcut-label step is unavailable on this device (Samsung Android 11 lacks `Ranking.getConversationShortcutInfo` despite its SDK level — the resolver catches the `LinkageError` once and seals the path), which makes the tag map the operative source there: seed it with `scripts/seed-room-name-map.sh`, which copies the system shortcut registry over adb, and re-run it when logcat shows `Group ... with no resolvable room name - dropping (tag=...)` for a brand-new room.
